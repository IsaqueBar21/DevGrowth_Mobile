...
export default function TortaEspumaLimao() {
    ...
    const formatarIngredientes = (ingredientes) => {
        return ingredientes.split("Base:").map((parte, index) => {
            if (index === 0) return null; // Ignorar a primeira parte, pois não tem a palavra 'Base'
            
            const [base, recheio] = parte.split("Recheio de Espuma de Limão:");
            return (
                <View key={index}>
                    <Text style={styles.subtitulo}>Base:</Text>
                    <Text style={styles.textoIngrediente}>{base.trim()}</Text>

                    <Text style={styles.subtitulo}>Recheio de Espuma de Limão:</Text>
                    <Text style={styles.textoIngrediente}>{recheio.trim()}</Text>
                </View>
            );
        });
    };
    ...
    const formatarPreparo = (preparo) => {
        return preparo.split("Base:").map((parte, index) => {
            if (index === 0) return null; // Ignorar a primeira parte, pois não tem a palavra 'Base'

            const [base, recheio] = parte.split("Recheio:");
            return (
                <View key={index}>
                    <Text style={styles.subtitulo}>Base:</Text>
                    <Text style={styles.textoIngrediente}>{base.trim()}</Text>

                    <Text style={styles.subtitulo}>Recheio:</Text>
                    <Text style={styles.textoIngrediente}>{recheio.trim()}</Text>
                </View>
            );
        });
